...
void init() {
...

    {
        blit::Pen tmppal[256];
        for (int i = 0; i < 256; ++i) {
            tmppal[i].r = (palette[i] >> 11) << 3;
            tmppal[i].g = ((palette[i] >> 5) & 0b111111) << 2;
            tmppal[i].b = (palette[i] & 0b11111) << 3;
            tmppal[i].a = i == 0 ? 0 : 255;
        }
        blit::set_screen_palette(tmppal, 256);
    }
